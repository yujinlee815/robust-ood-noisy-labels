...
class TXTImageDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, txt_file, image_root, transform=None):
        ...
        with open(txt_file) as f:
            for line in f:
                ...
                self.samples.append((osp.join(image_root, path), int(label)))
...
class TrainTask(object):
    ...
    def build_default_options():
        ...
        parser.add_argument('--entity', help='wandb entity (team or username)', type=str, default=None)
        ...
        parser.add_argument('--noise_type', type=str, default=None,
                            help='noise type key in CIFAR-N .pt file '
                                 '(cifar10n: aggre_label/worse_label/random_label1/2/3; '
                                 'cifar100n: noisy_label)')
    ...
    def create_dataset(data_root, dataset_name, train, transform=None, memory=False, label_file=None,
        ...
        if dataset_name in ['cifar10', 'cifar20', 'cifar100']:
            ...
            image_root = osp.join(data_root, 'images_classic')
            imglist_root = osp.join(data_root, 'benchmark_imglist', 'tinyimagenet')
            clean_txt = osp.join(imglist_root, 'train_tin.txt')
            if not train:
                dataset = TXTImageDataset(osp.join(imglist_root, 'val_tin.txt'), image_root, transform=transform)
            elif label_file is not None and label_file.endswith('.txt'):
                # 노이즈 txt를 직접 데이터 소스로 사용 (경로+노이즈 레이블)
                dataset = TXTImageDataset(label_file, image_root, transform=transform)
                clean_labels = np.array([int(l.strip().split()[1]) for l in open(clean_txt)])
                noise_ratio = 1 - np.mean(clean_labels == np.array(dataset.targets))
                print(f'Loaded noisy labels from {label_file}, noise ratio={noise_ratio:.4f}')
                label_file = None  # 이미 처리됨, 아래 공통 블록 스킵
            else:
                dataset = TXTImageDataset(clean_txt, image_root, transform=transform)
            has_subfolder = False
        else:
        ...
        if label_file is not None and train:
            if label_file.endswith('.pt'):
                data = torch.load(label_file, weights_only=False)
                if not isinstance(data, dict):
                    raise ValueError(f'Expected a dict in {label_file}, got {type(data)}')
                if noise_type is None:
                    raise ValueError(f'--noise_type must be specified for .pt label files. '
                                     f'Available keys: {list(data.keys())}')
                new_labels = np.array(data[noise_type])
            else:
                new_labels = np.load(label_file).flatten()
            assert len(dataset.targets) == len(new_labels), \
                f'label count mismatch: dataset={len(dataset.targets)}, file={len(new_labels)}'
            noise_ratio = 1 - np.mean(np.array(dataset.targets) == new_labels)
            dataset.targets = new_labels.tolist()
            print(f'Loaded noisy labels from {label_file} (noise_type={noise_type}), '
    ...
    def set_loader(self):
        ...
        if 'imagenet' in opt.dataset and opt.dataset != 'tinyimagenet':
    ...
    def fit(self):
        ...
        while True:
            ...
            if (self.cur_epoch % opt.save_freq == 0) or (self.cur_epoch == 1):
