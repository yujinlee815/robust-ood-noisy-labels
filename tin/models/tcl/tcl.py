...
class TCL(TrainTask):
    ...
    def set_model(self):
        ...
        encoder_type_raw, dim_in = backbone_dict[opt.encoder_name]
        encoder_type = partial(encoder_type_raw, num_classes=self.num_cluster)
    ...
    def build_options():
        ...
        parser.add_argument('--energy_loss_weight', type=float, default=0.001, help='energy_loss_weight')
    ...
    def train(self, inputs, indices, n_iter):
        ...
        contrastive_loss, cls_loss1, cls_loss2, ent_loss, ne_loss, align_loss, energy_loss = self.tcl(im_w, im_q, im_k, indices)
        ...
        loss = contrastive_loss + \
               ...
               opt.align_loss_weight * align_loss + opt.energy_loss_weight * energy_loss
        loss.backward()
        self.optimizer.step()
        #self.logger.msg([contrastive_loss, cls_loss1, cls_loss2, ent_loss, ne_loss, align_loss], n_iter)
        self.logger.msg([contrastive_loss, cls_loss1, cls_loss2, ent_loss, ne_loss, align_loss, energy_loss], n_iter)
    ...
    def evaluate(self, model, features, confidence, cluster_labels, labels, context_assignments, n_iter):
        ...
        if (self.cur_epoch % 50 == 0) or (self.cur_epoch == 1):
