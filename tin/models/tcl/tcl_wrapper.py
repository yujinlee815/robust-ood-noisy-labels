...
class Wrapper(nn.Module):
    ...
    @staticmethod
    def forward_energy_loss(logits, clean_mask, temperature=1.0):
        if clean_mask.sum() == 0:
            return logits.sum() * 0.0
        clean_logits = logits[clean_mask]
        energy = -temperature * torch.logsumexp(clean_logits / temperature, dim=1)
        return energy.mean()
...
class SimCLRWrapper(Wrapper):
    ...
    def inference(self, im_w, im_q, im_k):
        ...
        bs = im_w.size(0)
        #mix_randind = torch.randperm(bs).to('cuda')
        #alpha=self.mixup_alpha
        #mix_lam = np.random.beta(alpha, alpha)
        mix_randind = torch.arange(bs, device=im_w.device)
        mix_lam = torch.ones(bs, 1, device=im_w.device)
        
        # compute query features
        #x_q = self.encoder_q(torch.cat([im_w, im_mix, im_q, im_k]))  # queries: NxC
        x_q = self.encoder_q(torch.cat([im_w, im_q, im_k]))
        q = self.projector_q(x_q)  # queries: NxC
        q_logits = self.classifier_q(x_q)  # queries: NxC

        q = nn.functional.normalize(q, dim=1)  # already normalized
        q_w, q1, q2 = q.split([im_w.size(0), im_q.size(0), im_k.size(0)])
        w_logits, q_logits1, q_logits2 = q_logits.split([im_w.size(0), im_q.size(0), im_k.size(0)])

        # mix 없으므로 q_w, w_logits 재사용
        q_m, m_logits = q_w, w_logits
    ...
    def forward_loss(self, im_w, im_q, im_k, indices):
        ...
        clean_mask = self.confidences[indices] > 0.7
        energy_loss = self.forward_energy_loss(
            torch.cat([q_logits1, q_logits2], dim=0),
            clean_mask.repeat(2)
        )

        return contrastive_loss, cls_loss1, cls_loss2, ent_loss, ne_loss, align_loss, energy_loss
